...
public class EarthquakeCityMap extends PApplet {
	...
	private List<SimplePointMarker> markers;
	...
	public void setup() {
	    ...
	    for(PointFeature pf: earthquakes){
	    	//SimplePointMarker spm = new SimplePointMarker(pf.getLocation());
	    	SimplePointMarker spm = createMarker(pf);
	    	Object magObj = pf.getProperty("magnitude");
	    	float mag = Float.parseFloat(magObj.toString());
	    	if (mag >= THRESHOLD_MODERATE ){
	    		spm.setColor(red);
	    		spm.setRadius((float)12);
	    	}
	    	else if(mag < THRESHOLD_LIGHT){
	    		spm.setColor(blue);
	    		spm.setRadius((float)6);
	    	}
	    	else {
	    		spm.setColor(yellow);
	    		spm.setRadius((float)9);
	    	}
	    	markers.add(spm);
	    }
